---
title: "FlashTTS: Fast Streaming TTS with MTP Acceleration and X-pred Mean Flow Distillation"
title_zh: FlashTTS：结合MTP加速与X-pred均值流蒸馏的快速流式文本转语音
authors: "Hanke Xie, Xiaming Ren, Dake Guo, Ruonan You, Wenhao Li, Jingbin Hu, Guobin Ma, Huakang Chen, Kejie Xu, Rui Huang, Weiguo Tan, Xianrong Wang, Lei Xie"
date: 2026-06-08
pdf: "https://arxiv.org/pdf/2606.09141v2"
tags: ["query:speech-tech"]
score: 6.0
evidence: 快速流式文本转语音框架，低延迟
tldr: 现有单码本LLM TTS方法依赖多阶段流水线，缺乏流式支持，自回归预测慢且流匹配步数多，导致端到端延迟高。FlashTTS提出滞后多轨架构原生处理流式文本和语音输入，消除句子级缓冲；集成并行多token预测与X-pred均值流匹配解码器，仅需2次函数评估即可生成高保真mel谱。实验将首包延迟降至325ms，同时保持强零样本语音克隆和跨语言可懂度。该开源框架为实时语音对话系统提供了实用基础。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2606.09141v2/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 834, \"height\": 463}, {\"url\": \"assets/figures/arxiv/2606.09141v2/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 746, \"height\": 368}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2606.09141v2/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1492, \"height\": 367}, {\"url\": \"assets/tables/arxiv/2606.09141v2/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1489, \"height\": 357}, {\"url\": \"assets/tables/arxiv/2606.09141v2/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 744, \"height\": 479}, {\"url\": \"assets/tables/arxiv/2606.09141v2/table-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 839, \"height\": 268}]"
motivation: 现有TTS模型在实时语音对话中延迟高且缺乏对流式输入输出的原生支持，亟需低延迟流式方案。
method: 采用滞后多轨架构实现流式处理，结合并行多token预测与X-pred均值流匹配解码器，实现2步高保真声学生成。
result: 首包延迟降至325ms，显著低于流式基线，且保持零样本语音克隆与跨语言可懂度。
conclusion: FlashTTS通过联合优化输入处理与解码效率，为实时语音对话系统提供了低延迟流式TTS开源框架。
---

## 摘要
语音对话系统的最新进展要求文本转语音（TTS）模型更快、更具响应性。现代语音对话系统对TTS模型提出了两个主要要求：低延迟以及支持流式输入和输出。然而，大多数现有的基于单一码书的大型语言模型（LLM）的TTS方法依赖于多阶段流水线，缺乏原生流式能力。这些系统通常由于缓慢的自回归预测和多步流匹配而导致较高的端到端延迟。为了解决这些限制，我们提出了FlashTTS，一个开源、低延迟的流式TTS框架。FlashTTS引入了一种滞后多轨道架构，可原生处理流式文本和语音输入，从而消除对句子级缓冲的需求。为了加速声学生成，我们将并行多令牌预测（MTP）与X-pred均值流匹配解码器相结合。该配置在恰好两次函数评估（2-NFE）下实现了高保真令牌到梅尔谱的生成。通过联合优化输入处理和解码效率，FlashTTS为实时语音对话系统提供了实用基础。实验表明，与稳健的流式基线相比，FlashTTS将首包延迟显著降低至325毫秒，同时保持了强大的零样本声音克隆和跨语言可懂度。提供语音样本。模型代码和检查点将作为开源发布。

## Abstract
Recent progress in speech dialogue systems requires Text-to-Speech (TTS) models to be faster and more responsive. Modern speech dialogue systems impose two primary requirements on TTS models: low latency and support for streaming inputs and outputs. However, most existing single-codebook LLM-based TTS methods rely on multi-stage pipelines that lack native streaming capabilities. These systems typically suffer from high end-to-end latency due to slow autoregressive prediction and multi-step flow matching. To address these limitations, we propose FlashTTS, an open-source and low-latency streaming TTS framework. FlashTTS introduces a lagged multi-track architecture that natively processes streaming text and speech inputs, thereby eliminating the need for sentence-level buffering. To accelerate acoustic generation, we integrate parallel Multi-Token Prediction (MTP) with an X-pred mean flow matching decoder. This configuration achieves high-fidelity token-to-mel generation in exactly two function evaluations (2-NFE). By jointly optimizing input processing and decoding efficiency, FlashTTS offers a practical foundation for real-time speech dialogue systems. Experiments show that FlashTTS substantially reduces First-Packet Latency to 325ms compared to robust streaming baselines, all while preserving strong zero-shot voice cloning and cross-lingual intelligibility. Speech samples are available. The model code and checkpoints will be released as open source.