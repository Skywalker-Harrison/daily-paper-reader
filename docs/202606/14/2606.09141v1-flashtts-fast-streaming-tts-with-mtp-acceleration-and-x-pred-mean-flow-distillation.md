...
title_zh: "FlashTTS: 结合MTP加速与X-pred均值流蒸馏的快速流式TTS"
...
score: 6.0
evidence: 快速流式文本转语音
tldr: 现有单码本LLM TTS方法依赖多阶段流水线，延迟高且缺乏原生流式能力。FlashTTS提出滞后多轨道架构原生支持流式输入，并行多令牌预测加速自回归解码，X-pred均值流匹配解码器实现2步高质量声学生成。实验表明首包延迟降低至325ms，显著优于流式基线，同时保持零样本语音克隆和跨语言可懂度。该框架为实时语音对话系统提供了高效低延迟的流式TTS解决方案，将开源代码和模型。
source: arxiv
...
motivation: 现有单码本LLM TTS方法延迟高、缺乏原生流式能力，无法满足实时语音对话系统的低延迟和流式需求。
method: 提出滞后多轨道架构原生处理流式文本和语音，集成并行多令牌预测加速解码，结合X-pred均值流匹配实现2-NFE高质量声学生成。
result: 首包延迟降至325ms，显著低于流式基线，同时保持强零样本语音克隆和跨语言可懂度。
conclusion: FlashTTS通过联合优化输入流处理和解码效率，为实时语音对话系统提供了低延迟、高保真的流式TTS框架。
---

## 摘要
近期语音对话系统的进展要求文本转语音（TTS）模型更快、响应更及时。现代语音对话系统对TTS模型提出了两个主要要求：低延迟以及对流式输入和输出的支持。然而，现有的基于单码本LLM的TTS方法大多依赖于缺乏原生流式能力的多阶段流水线。这些系统通常由于缓慢的自回归预测和多步流匹配而导致端到端延迟较高。为了解决这些限制，我们提出了FlashTTS，一个开源的低延迟流式TTS框架。FlashTTS引入了一种延迟多轨道架构，能够原生处理流式文本和语音输入，从而消除了句子级缓冲的需求。为了加速声学生成，我们将并行多令牌预测（MTP）与X-pred均值流匹配解码器相结合。该配置恰好通过两次函数评估（2-NFE）实现了高保真度的令牌到梅尔频谱的生成。通过联合优化输入处理和解码效率，FlashTTS为实时语音对话系统提供了实用基础。实验表明，与稳健的流式基线相比，FlashTTS将首包延迟显著降低至325毫秒，同时保持了强大的零样本人声克隆和跨语言可懂度。语音样本可获取。模型代码和检查点将作为开源发布。
